...
async function generateContentWithRetry(payload, retries = 2) {
    try {
        ...
        const isTemporaryError = error?.status === 503 ||
    ...
    try {
        ...
        if (isHighDemandError) {
            return res.status(503).json({
                message: "Our AI analysis service is currently experiencing high traffic. Please try again in a few minutes.",
            });
        }
        return res.status(500).json({
...
export const generateInterview = TryCatch(async (req, res) => {
    ...
    try {
        ...
        if (isHighDemandError) {
            return res.status(503).json({
                message: "Generating Interview Questions are temporarily unavailable due to high demand. Please try again in a few minutes.",
            });
        }
        return res.status(500).json({
            message: "Unable to generate interview question at the moment. Please try again later.",
        });
    }
});
// API to Build Resume
export const buildResume = TryCatch(async (req, res) => {
    ...
    try {
        ...
        if (isHighDemandError) {
            return res.status(503).json({
                message: "Resume Builder is temporarily unavailable due to high demand. Please try again in a few minutes.",
            });
        }
        return res.status(500).json({
